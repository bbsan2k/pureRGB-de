_CeruleanCityRivalPreBattleText::
	text "<RIVAL>: Hi,"
	line "<PLAYER>!"

	para "Du bist also"
	line "immer noch "
	cont "unterwegs!"

	para "Mir geht es"
	line "bestens! Ich habe"
	cont "jede Menge neue,"
	cont "starke #MON"
	cont "gefangen!"

	para "Zeig mal, was Du"
	line "gefangen hast,"
	cont "<PLAYER>!"
	done

_CeruleanCityRivalDefeatedText::
	text "Hey, "
	line "schon gut! Du"
	cont "hast gewonnen!"
	prompt

_CeruleanCityRivalVictoryText::
	text "Ha!"
	line "Meiner Genialität"
	cont "kannst Du nichts"
	cont "entgegensetzen!"
	prompt

_CeruleanCityRivalIWentToBillsText::
	text "<RIVAL>: Soll"
	line "ich Dir was"
	cont "verraten?"

	para "Ich war bei BILL"
	line "und habe mir"
	cont "seine seltenen"
	cont "#MON"
	cont "zeigen lassen!"

	para "Das hat mir"
	line "einige Einträge"
	cont "in meinem #DEX"
	cont "gebracht!"

	para "BILL ist weltweit"
	line "als #MANIAC"
	cont "bekannt!"

	para "Er hat das "
	line "#MON- "
	cont "Lagersystem für"
	cont "den PC"
	cont "entwickelt!"

	para "Du kannst Dich"
	line "bei ihm dafür"
	cont "bedanken!"

	para "Naja, ich hau ab!"
	line "Mach's mal gut!"
	done

_CeruleanCityRocketText::
	text "He! Bleib"
	line "draußen! Du hast"
	cont "hier nichts zu"
	cont "suchen! Was ich"
	cont "hier tue?"

	para "Ich bin nur ein"
	line "unschuldiger"
	cont "Passant! Was?"
	cont "Glaubst Du mir"
	cont "etwa nicht?"
	done

_CeruleanCityRocketReceivedTM28Text::
	text "<PLAYER> erhält"
	line "TM28!@"
	text_end

_CeruleanCityRocketIBetterGetMovingText::
	text_start

	para "Ich gehe wohl"
	line "besser! Tschüß!@"
	text_end

_CeruleanCityRocketTM28NoRoomText::
	text "Mache Platz für"
	line "dieses coole"
	cont "Item!"

	para "Ich kann erst"
	line "gehen, wenn ich"
	cont "es Dir gegeben"
	cont "habe!"
	done

_CeruleanCityRocketIGiveUpText::
	text "Ist gut!"
	line "Ich gebe auf!"
	cont "Ich verschwinde!"
	prompt

_CeruleanCityRocketIllReturnTheTMText::
	text "OK! Ich gebe die"
	line "gestohlene TM"
	cont "zurück!"
	prompt

_CeruleanCityCooltrainerMText::
	text "Du bist auch ein"
	line "Trainer? Zu"
	cont "sammeln und zu"
	cont "kämpfen ist nicht"
	cont "immer so leicht!"
	done

_CeruleanCitySuperNerd1Text::
	text "Der Busch vor dem"
	line "Laden steht im"
	cont "Weg!"

	para "Vielleicht gibt es"
	line "einen anderen"
	cont "Weg!?"
	done

_CeruleanCitySuperNerd1_AfterTreeDelete::
	text "Ein verrückter"
	line "Typ mit Sonnen-"
	cont "brille flog"
	cont "hier durch wie"
	cont "ein Wirbelwind!"
	para "Dann hat sein"
	line "Relaxo einfach"
	cont "diesen Baum, der"
	cont "vor dem Laden"
	cont "stand entwurzelt!"
	para "Sie verschwanden"
	line "so schnell wie"
	cont "sie kamen!"
	para "Was ein Anblick!"
	done

_CeruleanCitySuperNerd2Text::
	text "Du erstellst eine"
	line "Enzyklopädie der"
	cont "#MON? Das"
	cont "klingt lustig!"
	done

_CeruleanCityGuardText::
	text "Die Leute hier"
	line "wurden beraubt."

	para "Das TEAM ROCKET"
	line "steckt hinter den"
	cont "abscheulichen"
	cont "Verbrechen!"

	para "Sogar die POLIZEI"
	line "ist gegen das"
	cont "TEAM ROCKET"
	cont "machtlos!"
	done

_CeruleanCityCooltrainerF1SlowbroUseSonicboomText::
	text "OK! LAHMUS!"
	line "Setze ULTRASCHALL"
	cont "ein! Los, mach"
	cont "schon, LAHMUS!"
	done

_CeruleanCityCooltrainerF1SlowbroPunchText::
	text "LAHMUS, schlage"
	line "zu! Oh nein!"
	cont "Wieder nicht..."
	done

_CeruleanCityCooltrainerF1SlowbroWithdrawText::
	text "LAHMUS, ZURÜCK!"
	line "Nein, nicht!!!"

	para "Es ist schwer,"
	line "ein #MON"
	cont "zu kontrollieren!"

	para "Ob Dein #MON"
	line "gehorcht,  hängt"
	cont "von Deinen"
	cont "Fähigkeiten als"
	cont "Trainer ab!"
	done

_CeruleanCitySlowbroTookASnoozeText::
	text "LAHMUS macht"
	line "ein Nickerchen..."
	done

_CeruleanCitySlowbroIsLoafingAroundText::
	text "LAHMUS bummelt"
	line "herum..."
	done

_CeruleanCitySlowbroTurnedAwayText::
	text "LAHMUS wendet"
	line "sich ab..."
	done

_CeruleanCitySlowbroIgnoredOrdersText::
	text "LAHMUS ignoriert"
	line "die Befehle..."
	done

_CeruleanCityCooltrainerF2Text::
	text "Ich hätte gern"
	line "ein rotes"
	cont "FAHRRAD!"

	para "Aber ich würde"
	line "nicht damit"
	cont "fahren, damit es"
	cont "nicht schmutzig"
	cont "wird!"
	done

_CeruleanCitySuperNerd3Text::
	text "Die AZURIA"
	line "HÖHLE! Dort leben"
	cont "extrem starke"
	cont "#MON!"

	para "Nur PKMN-LIGA-"
	line "Champions ist der"
	cont "Zutritt erlaubt!"
	done

_CeruleanCitySignText::
	text "AZURIA CITY"
	line "Eine mysteriöse"
	cont "blaue Aura"
	cont "umgibt die Stadt!"
	done

_CeruleanCityTrainerTipsText::
	text "TIPS FÜR TRAINER"

	para "Drückt man den"
	line "B-Knopf während"
	cont "der Entwicklung"
	cont "eines #MON, so"
	cont "wird der Prozess"
	cont "abgebrochen!"
	done

_CeruleanCityBikeShopSign::
	text "FAHRRADLADEN"
	line "Fahre leicht und"
	cont "schnell mit dem"
	cont "Rad durch Höhlen"
	cont "und Gras!"
	done

_CeruleanCityGymSign::
	text "AZURIA CITY"
	line "PKMN-ARENA"
	cont "ARENALEITERIN: "
	cont "MISTY"

	para "Die gruselige"
	line "Arenaleiterin!"
	done


_OakBattleStartText::
	text "Hallo, <PLAYER>!"

	para "Wie läufts mit"
	line "deinem POKéDEX?"

	para "Was ich hier"
	line "mache?"

	para "Trainieren,"
	line "natürlich!"

	para "Ich komme einmal"
	line "die Woche zum"
	cont "Trainieren her"
	cont "seit ich in"
	cont "deinem Alter"
	cont "war!"

	para "Zum drittek"
	line "Start-#MON:"

	para "Ich ziehe es"
	line "groß seit du und"
	cont "<RIVAL> los-"
	cont "gezogen seid!"

	para "Es ist zu einem"
	line "wundervollen"
	cont "Kämpfer geworden!"

	para "Ich zeige dir,"
	line "wie wir früher"
	cont "#MON-Kämpfe"
	cont "ausgetragen"
	cont "haben!"
	done

_OakBattleWinText::
	text "Be-"
	line "Beachtlich!"
	prompt

_OakBattleLoseText::
	text "Sieht aus als"
	line "ob ich es immer"
	cont "noch drauf hab!"
	prompt

_OakBeatenText::
	text "Der Job eines"
	line "Trainers ist"
	cont "niew vorbei!"

	para "Du und dein"
	line "#MON solltet"
	cont "immer versuchen"
	cont "euch zu"
	cont "verbessern!"

	para "Komm gerne zum"
	line "Trainieren"
	cont "vorbei!"

	para "Ich habe ein"
	line "paare tolle"
	cont "Geschichten zu"
	cont "erzählen!"
	done

_OakFirstLoseText::
	text "Toller Kampf!"
	para "Wie vom CHAMPION"
	line "zu erwarten war!"
	para "Ich denke ich"
	line "sollte dich"
	cont "belohnen."
	para "Ah, wie wärs"
	line "damit?"
	para "Nimm diese Notiz"
	line "mit ins Labor der"
	cont "ZINNOBERINSEL."
	para "Mein Kollege"
	line "arbeitet dort!"
	para "Er weiß was"
	line "zutun ist!"
	para "..."
	para "..."
	para "..."
	para "PROF.EICH gibt"
	line "dir eine Notiz!@"
	text_end